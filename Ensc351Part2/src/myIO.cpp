...
struct sockInfo {
	...
	mutable std::condition_variable cvW;	// used to wait and notify threads blocked on myTcdrain(), i.e., threads that has called write()
	mutable std::condition_variable cvR;	// used to wait and notify threads blocked on myReadcond(), i.e., threads that has called myRead() or myReadcond()
...
ssize_t myRead(int fildes, void* buf, size_t nbyte) {
	// synchronizations are moved to myReadcond() in case of user calling myReadcond() directly
...
ssize_t myWrite(int fildes, const void* buf, size_t nbyte) {\
	...
	if (writeSock && (pairedSock = getSockInfo(writeSock->pairedFd))) {
		...
		// Note that in this case we are releasing mutex before notifying a potential blocked reader to be efficient
		// This is ok because another thread of reader() or writer() holding mutex before a reader getting notified is perfectly fine
		// See myReadcond() for analysis on efficiency
		pairedSock->cvR.notify_all();
	} else
		retVal = write(fildes, buf, nbyte);
...
// However, threads that are waiting on tcDrain(pairedFd)::pairedSock->cvW.wait() should be waken up.
int myClose(int fd) {
	int retVal;
	...
	if (closeSock) {
		...
		closeSock->cvW.notify_all();	// free the remaining tcDrain(fd)s
		closeSock->cvR.notify_all();	// free the remaining read(fd)s
...
int myTcdrain(int des) { //is also included for purposes of the course.
	...
	if (sockInfo* sock = getSockInfo(des)) {
		...
		if (sock->buffCnter) {
//			sock->cvW.wait(lk, [] {return !(sock->buffCnter);});
...
int myReadcond(int des, void * buf, int n, int min, int time, int timeout) {
	...
	sockInfo* readSock = getSockInfo(des);
	...
	// If myReadcond() is blocked waiting to read,and then the socket closes. It will return with -1 (not hang)

	if (!readSock) {	// error or non-socket: do not continue
		retVal = wcsReadcond(des, buf, n, min, time, timeout);
#ifdef _DEBUG
		if (retVal == -1)
			std::cerr << "myRead(" << des << ", buf, " << n << ") failed"
					<< std::endl;
#endif
	} else {
		{
			// set block the thread until there are buffCnter > 0
			std::unique_lock<std::mutex> lk(readSock->buffMut);
			if(readSock->buffCnter == 0){
				readSock->cvR.wait(lk);
			}
			// Now buffCnter > 0 && thread holds the mutex
			retVal = wcsReadcond(des, buf, n, min, time, timeout);
			readSock->buffCnter = readSock->buffCnter - retVal;

			// Holding readSock->buffMut for notify()ing is NOT inefficient because we are not holding pairedSock->buffMut
			 ...
			 * threadA::myWrite(pairedFd) && set buffCnter to n bytes -> threadA::tcDrain()::wait(pairedFd) -> threadB::myRead(fd) resets buffCnter=0 and release buffMut
			 *  -> threadC::myWrite(pairedFd) && set buffCnter to n bytes again -> threadC::tcDrain::wait(pairedFd) -> threadB::myRead()::notify_all().
			...
			*/
			if (!(readSock->buffCnter)) {
				...
				if(pairedSock){		// pairedSock==nullptr --> socket already closed. Nothing to do.
					...
					pairedSock->cvW.notify_all();
				}
			}
			else{	// still chars in buffer, notify one more
				// note that despite being inefficient(as explained above), it is crucial for precise scheduling:
				// Simple scenario:
				/* another threadB.myWrite(fd, nBytes) comes before this notify_one(cv_R)
				 * The myRead() thread that got woken up will see nBytes+actualBytes before read
				 */
				readSock->cvR.notify_one();

			}
			// lk(readSock->buffMut) unlocks upon out of scope
		}
	}
	return retVal;
}
